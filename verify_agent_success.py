...
def main():
            ...
            for var in loadset_vars:
                if 'LoadSet' in variables['tool_result']['variables'][var]['type']:
                    ...
                    if var_detail['success']:
                        print(f"\n📊 {var} details:")
                        print(f"  Type: {var_detail['tool_result']['variable_info']['type']}")
                        print(f"  Value preview: {var_detail['tool_result']['variable_info']['value'][:200]}...")
    ...
    successful_elements = sum(1 for element, _ in workflow_elements if element.lower() in all_code)
    print(f"  - Workflow completeness: {successful_elements}/{len(workflow_elements)} elements")
    print(f"  - Code execution success rate: {sum(1 for e in executions if e['success'])}/{len(executions)}")
