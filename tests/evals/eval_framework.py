    ...
    def generate_report(self, results: list[EvalResult]) -> dict[str, Any]:
        ...
        return {
            ...
            "results": [
                ...
                for result, evaluator, eval_case in zip(
                    results,
                    self.evaluators * len(self.eval_cases),
                    self.eval_cases * len(self.evaluators),
                )
            ],
        }

    def save_report(self, results: list[EvalResult], output_path: str | Path):
...
class ToolCallCapturingTool:
    ...
    def _wrap_function(self, fn):
        ...
        def wrapper(**kwargs):
            start_time = time.time()
            tool_call = ToolCall(
                name=self._tool_name, args=kwargs.copy(), timestamp=start_time
            )
