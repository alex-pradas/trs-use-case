...
class ToolCallEvaluator(BaseEvaluator):
    ...
    def __init__(
        ...
        self.allow_extra_calls = (
            allow_extra_calls  # If True, allows additional tool calls
        )
        self.runner = MockableAgentRunner(capture_tool_calls=True)

    async def evaluate(
        self, agent: Agent, eval_case: EvalCase, deps: Any
    ) -> EvalResult:
        ...
        return self._evaluate_tool_calls(
            captured_calls, eval_case.expected_tool_calls, result.output
        )
                ...
                detailed_results.append(
                    {
                        ...
                        "score": self._calculate_call_score(
                            best_match, tool_name, expected_args
                        ),
                    }
                )
            else:
    ...
    def _calculate_call_score(
        self, call: ToolCall, expected_name: str, expected_args: dict[str, Any]
    ) -> float:
    ...
    def _create_summary_message(
        self, matched: int, expected: int, actual_total: int, score: float
    ) -> str:
...
class SpecificToolEvaluator(ToolCallEvaluator):
    ...
    def __init__(
        self, tool_name: str, expected_args: dict[str, Any], name: str | None = None
    ):
    ...
    async def evaluate(
        ...
        eval_case.expected_tool_calls = [
            {"name": self.tool_name, "args": self.expected_args}
        ]
...
class ScaleLoadsEvaluator(SpecificToolEvaluator):
    ...
    def __init__(self, expected_factor: float, name: str | None = None):
        super().__init__(
    ...
    async def evaluate(
        ...
        if result.passed:
            result.message = (
                f"✅ scale_loads called with factor {self.expected_factor} as expected"
            )
        else:
            # Check if scale_loads was called at all
            scale_calls = [
                call for call in result.tool_calls if call.name == "scale_loads"
            ]
            if scale_calls:
                actual_factors = [
                    call.args.get("factor", "unknown") for call in scale_calls
                ]
...
class ToolSequenceEvaluator(BaseEvaluator):
    ...
    def __init__(
        self, expected_sequence: list[str], name: str = "ToolSequenceEvaluator"
    ):
        super().__init__(name)
    ...
    async def evaluate(
        ...
        message = self._create_sequence_message(
            actual_sequence, self.expected_sequence, score
        )
        ...
        return EvalResult(
            ...
            details={
                ...
                "sequence_found": self._find_subsequence(
    ...
    def _create_sequence_message(
        self, actual: list[str], expected: list[str], score: float
    ) -> str:
