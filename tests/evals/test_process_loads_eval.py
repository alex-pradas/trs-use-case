...
from tests.evals.tool_call_eval import ScaleLoadsEvaluator, ToolCallEvaluator, ToolSequenceEvaluator
...
the files are here: solution/loads/new_loads.json
    ...
    async def test_scale_loads_factor_1_5_evaluation(
        ...
        evaluator = ScaleLoadsEvaluator(expected_factor=1.5, name="UltimateLoadFactorEval")
    ...
    async def test_comprehensive_tool_call_evaluation(
        ...
        factor_1_5_calls = [
            call for call in scale_calls 
            if call.args.get("factor") == 1.5
        ]
        assert factor_1_5_calls, f"scale_loads not called with factor 1.5. Found factors: {[call.args.get('factor') for call in scale_calls]}"
    ...
    async def test_original_prompt_evaluation(
        self, 
        process_loads_agent, 
        original_user_prompt_eval_case, 
        mcp_dependencies
    ):
        ...
        scale_calls = [call for call in result.tool_calls if call.name == "scale_loads"]
        if scale_calls:
            print(f"✅ scale_loads called with factors: {[call.args.get('factor') for call in scale_calls]}")
        else:
    ...
    async def test_tool_sequence_evaluation(
        ...
        evaluator = ToolSequenceEvaluator(
            ...
            name="LoadProcessingSequenceEval"
        )
    ...
    async def test_evaluation_suite_run(
        ...
        assert report['pass_rate'] >= 0.5, f"Too many evaluations failed: {report['pass_rate']:.2%}"
...
class TestProcessLoadsAgentBehavior:
    ...
    async def test_agent_follows_system_prompt_guidance(
        self, 
        loadset_agent_with_custom_prompt, 
        mcp_dependencies
    ):
        ...
        prompt = "Process loads for ultimate analysis from solution/loads/new_loads.json"
    ...
    async def test_no_double_factoring(
        ...
        prompt = "Load solution/loads/new_loads.json and apply ultimate safety factor of 1.5"
