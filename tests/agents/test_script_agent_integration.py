...
"""
    ...
    async def test_agent_basic_script_generation(self):
        ...
        assert result["success"], (
            f"Agent processing failed: {result.get('error', 'Unknown error')}"
        )
        ...
        try:
            ...
            assert isinstance(content, (dict, list)), (
                "JSON content should be dict or list"
            )
    ...
    async def test_agent_loadset_basic_processing(self):
        ...
        assert result["success"], (
            f"LoadSet processing failed: {result.get('error', 'Unknown error')}"
        )
        ...
        assert len(json_files) >= 1, (
            f"Expected JSON files but got: {[f['local_path'] for f in downloaded_files]}"
        )
            ...
            if json_path.exists():
                try:
                    ...
                    if isinstance(content, dict) and "load_cases" in content:
                        ...
                        assert "force_units" in content, (
                            "LoadSet should have force units"
                        )
                        assert len(content["load_cases"]) > 0, (
                            "LoadSet should have load cases"
                        )
                        break
                except json.JSONDecodeError:
                    continue
    ...
    async def test_agent_loadset_unit_conversion_workflow(self):
        ...
        assert result["success"], (
            f"Unit conversion workflow failed: {result.get('error', 'Unknown error')}"
        )
        ...
        assert len(json_files) >= 2, (
            f"Expected multiple JSON files but got: {len(json_files)}"
        )
        ...
        assert len(unit_systems_found) >= 2, (
            f"Expected multiple unit systems but found: {unit_systems_found}"
        )
    ...
    async def test_agent_loadset_comparison_workflow(self):
        ...
        assert result["success"], (
            f"LoadSet comparison failed: {result.get('error', 'Unknown error')}"
        )
    ...
    async def test_agent_loadset_ansys_export_workflow(self):
        ...
        assert result["success"], (
            f"ANSYS export workflow failed: {result.get('error', 'Unknown error')}"
        )
        ...
        assert len(inp_files) > 0, (
            f"Expected ANSYS .inp files but got file types: {[Path(f['local_path']).suffix for f in downloaded_files]}"
        )
        ...
        for inp_file_info in inp_files:
            ...
            if inp_path.exists():
                ...
                assert "f,all," in content, (
                    f"ANSYS file {inp_path} should contain f,all commands"
                )
                assert any(cmd in content for cmd in ["fx,", "fy,", "fz,"]), (
                    f"ANSYS file {inp_path} should contain force commands"
                )
                break
        else:
    ...
    async def test_agent_error_handling_and_recovery(self):
        ...
        assert result["success"], (
            f"Agent should recover from errors but failed: {result.get('error', 'Unknown error')}"
        )
    ...
    async def test_agent_custom_analysis_workflow(self):
        ...
        assert result["success"], (
            f"Custom analysis workflow failed: {result.get('error', 'Unknown error')}"
        )
        ...
        assert analysis_found or len(text_files) > 0, (
            "No analysis results found in downloaded files"
        )
    ...
    def test_agent_available_tools(self):
        ...
        assert tools_result["success"], (
            f"Failed to get available tools: {tools_result.get('error')}"
        )
