                ...
                LoadCase(
                    ...
                    point_loads=[
                        PointLoad(
                            name="Point_Zero",
                            force_moment=ForceMoment(
                                fx=0.0, fy=0.0, fz=0.0,  # All forces zero
                                mx=10.0, my=0.0, mz=0.0  # Only mx non-zero
                            ),
                        ),
                    ],
                ),
...
class TestLoadSetComparison:
    ...
    def test_compare_to_basic(self):
        ...
        fx_rows = [row for row in comparison.comparison_rows 
    ...
    def test_compare_to_different_units(self):
            ...
            load_cases=[
                LoadCase(
                    ...
                    point_loads=[
                        PointLoad(
                            name="Point_A",
                            force_moment=ForceMoment(fx=0.12, fy=0.18, fz=0.30),  # 120N, 180N, 300N in kN
                        ),
                    ],
                ),
            ],
        ...
        fx_max_row = next((row for row in comparison.comparison_rows 
                          if row.point_name == "Point_A" and row.component == "fx" and row.type == "max"), None)
    ...
    def test_compare_to_invalid_input(self):
        ...
        with pytest.raises(ValueError, match="Can only compare to another LoadSet instance"):
...
class TestLoadSetComparisonWithRealData:
    ...
    def test_compare_real_data_files(self):
        ...
        if self.old_loads_path.exists() and self.new_loads_path.exists():
            ...
            if len(comparison.comparison_rows) >= 2:
                ...
                print(f"Sample row: {sample_row.point_name}.{sample_row.component}.{sample_row.type} - LoadSet1: {sample_row.loadset1_value}, LoadSet2: {sample_row.loadset2_value}, Diff: {sample_row.pct_diff:.1f}%")
        else:
    ...
    def test_compare_to_percentage_calculations(self):
            ...
            load_cases=[
                LoadCase(
                    ...
                    point_loads=[
                        PointLoad(
                            name="Point_A",
                            force_moment=ForceMoment(fx=100.0, fy=200.0),  # Both non-zero
                        ),
                    ],
                ),
            ],
        ...
        fx_row = next((row for row in comparison.comparison_rows 
                      if row.point_name == "Point_A" and row.component == "fx"), None)
