...
class TestLoadSetConvertTo:
    ...
    def test_convert_to_lbf(self):
        ...
        assert abs(point_a.force_moment.my - 147.513) < 0.001  # 200 Nm -> ~147.513 lbf-ft
        assert abs(point_a.force_moment.mz - 221.269) < 0.001  # 300 Nm -> ~221.269 lbf-ft
    ...
    def test_convert_to_klbf(self):
        ...
        point_a = converted.load_cases[0].point_loads[0]
        assert abs(point_a.force_moment.fx - 0.224809) < 0.000001  # 1000 N -> ~0.224809 klbf
        assert abs(point_a.force_moment.fy - 0.449618) < 0.000001  # 2000 N -> ~0.449618 klbf
        assert abs(point_a.force_moment.fz - 0.674427) < 0.000001  # 3000 N -> ~0.674427 klbf
    ...
    def test_convert_preserves_metadata(self):
        ...
        assert converted.load_cases[0].description == self.sample_loadset.load_cases[0].description
        
        # Check point load metadata
        assert len(converted.load_cases[0].point_loads) == len(self.sample_loadset.load_cases[0].point_loads)
        assert converted.load_cases[0].point_loads[0].name == self.sample_loadset.load_cases[0].point_loads[0].name
        assert converted.load_cases[0].point_loads[1].name == self.sample_loadset.load_cases[0].point_loads[1].name
    ...
    def test_convert_chain_conversions(self):
        ...
        assert abs(point_a_chain.force_moment.fx - point_a_direct.force_moment.fx) < 0.001
        assert abs(point_a_chain.force_moment.fy - point_a_direct.force_moment.fy) < 0.001
        assert abs(point_a_chain.force_moment.fz - point_a_direct.force_moment.fz) < 0.001
...
class TestLoadSetFactor:
    ...
    def test_factor_preserves_metadata(self):
        ...
        for i, (orig_case, fact_case) in enumerate(zip(self.sample_loadset.load_cases, factored.load_cases)):
            ...
            for j, (orig_point, fact_point) in enumerate(zip(orig_case.point_loads, fact_case.point_loads)):
            ...
            load_cases=[
                LoadCase(
                    ...
                    point_loads=[
                        PointLoad(
                            name="Node-1001",
                            force_moment=ForceMoment(fx=100.0, fy=0.0, fz=0.0, mx=0.0, my=0.0, mz=0.0)
                        )
                    ]
                )
            ]
