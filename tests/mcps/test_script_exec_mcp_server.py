...
class TestScriptExecutorMCPProvider:
    ...
    def setup_method(self):
        ...
        self.provider = ScriptExecutorMCPProvider(
            base_workspace_dir=self.temp_dir, execution_timeout=30
        )
...
"""
        ...
        result = self.provider.execute_python_script(
            script, include_loadset_imports=True
        )
    ...
    def test_script_without_loadset_imports(self):
        ...
        result = self.provider.execute_python_script(
            script, include_loadset_imports=False
        )
...
"""
        ...
        assert any(
            "subdir/file2.txt" in path or "subdir\\file2.txt" in path
...
"""
    ...
    def test_upload_file_with_subdirectory(self):
        ...
        result = self.provider.upload_file(
            "subdir/nested.txt", content, encoding="text"
        )
...
class TestMCPServerCreation:
    ...
    def test_server_with_custom_params(self):
        ...
        try:
            server = create_mcp_server(
                base_workspace_dir=temp_dir, execution_timeout=60
            )
...
class TestLoadSetIntegration:
    ...
    def setup_method(self):
        ...
        self.temp_dir = Path(tempfile.mkdtemp())
        self.provider = ScriptExecutorMCPProvider(
            base_workspace_dir=self.temp_dir, execution_timeout=60
        )
