...
from tools.response_models import (
    LoadSetResponse, ConversionResponse, ExecutionResponse, ScriptResponse,
    ComparisonResponse, ExportResponse, SessionResponse, FileOperationResponse, WorkspaceResponse
)
...
def create_loadset_agent() -> Agent[MCPServerProvider, str]:
    ...
    agent = Agent(
        ...
        system_prompt="You are an expert aerospace structural loads analyst with access to LoadSet processing tools. Use available tools for operations and provide clear explanations.",
    )
    ...
    def load_from_json(ctx: RunContext[MCPServerProvider], file_path: str) -> LoadSetResponse:
        ...
        return LoadSetResponse(
            success=True, message=f"LoadSet loaded from {file_path}", data=result,
            load_cases_count=len(result.get("load_cases", [])) if result else None
        )
    
    @agent.tool
    def convert_units(ctx: RunContext[MCPServerProvider], target_units: str) -> ConversionResponse:
        """Convert the current LoadSet to different units."""
        result = ctx.deps.loads_server._tool_manager._tools["convert_units"].fn(target_units=target_units)
        return ConversionResponse(
            success=True, message=f"Units converted to {target_units}",
            original_units=result.get("original_units", {}), target_units=result.get("new_units", {}),
            conversion_factor=result.get("conversion_factor", 1.0)
        )
    
    @agent.tool
    def scale_loads(ctx: RunContext[MCPServerProvider], factor: float) -> LoadSetResponse:
        """Scale all loads in the current LoadSet by a factor."""
        result = ctx.deps.loads_server._tool_manager._tools["scale_loads"].fn(factor=factor)
        return LoadSetResponse(success=True, message=f"Loads scaled by factor {factor}", data=result)
    
    @agent.tool
    def export_to_ansys(ctx: RunContext[MCPServerProvider], folder_path: str, name_stem: str) -> ExportResponse:
        """Export the current LoadSet to ANSYS format files."""
        result = ctx.deps.loads_server._tool_manager._tools["export_to_ansys"].fn(folder_path=folder_path, name_stem=name_stem)
        return ExportResponse(
            success=True, message="LoadSet exported to ANSYS format",
            files_created=result.get("files_created", []), export_format="ANSYS",
            output_location=folder_path, file_count=len(result.get("files_created", []))
        )
    ...
    def compare_loadsets(ctx: RunContext[MCPServerProvider]) -> ComparisonResponse:
        ...
        return ComparisonResponse(
            success=True, message="LoadSet comparison completed",
...
def create_python_agent() -> Agent[MCPServerProvider, str]:
    ...
    def list_variables(ctx: RunContext[MCPServerProvider]) -> SessionResponse:
        ...
        return SessionResponse(
            success=True, message="Variables listed successfully", session_state="active",
    ...
    def reset_session(ctx: RunContext[MCPServerProvider]) -> SessionResponse:
        ...
        return SessionResponse(success=True, message="Session reset successfully", session_state="reset", execution_count=0)
...
def create_script_agent() -> Agent[MCPServerProvider, str]:
    ...
    @agent.tool
    def execute_python_script(ctx: RunContext[MCPServerProvider], script_content: str, script_name: str = "generated_script.py") -> ScriptResponse:
        """Execute a complete Python script in isolated workspace."""
        result = ctx.deps.script_server._tool_manager._tools["execute_python_script"].fn(script_content=script_content, script_name=script_name)
        execution_result = result.get("execution_result", {})
        return ScriptResponse(
            success=execution_result.get("success", False),
            message="Script executed successfully" if execution_result.get("success") else "Script execution failed",
    ...
    @agent.tool
    def download_file(ctx: RunContext[MCPServerProvider], file_path: str, output_format: str = "auto") -> FileOperationResponse:
        """Download files from workspace as base64 or text."""
        result = ctx.deps.script_server._tool_manager._tools["download_file"].fn(file_path=file_path, encoding=output_format)
        return FileOperationResponse(
            success=result.get("success", False),
            message="File downloaded successfully" if result.get("success") else "File download failed",
            file_path=file_path, file_size=result.get("size", None), encoding=result.get("encoding", None),
    ...
    def reset_workspace(ctx: RunContext[MCPServerProvider]) -> WorkspaceResponse:
        ...
        return WorkspaceResponse(
            success=result.get("success", False),
            message="Workspace reset successfully" if result.get("success") else "Workspace reset failed",
            workspace_path=None, files_count=0, is_active=False
        )
...
__all__ = [
    "create_loadset_agent", "create_python_agent", "create_script_agent",
    "LoadSetResponse", "ConversionResponse", "ExecutionResponse", "ScriptResponse",
    "ComparisonResponse", "ExportResponse", "SessionResponse", "FileOperationResponse", "WorkspaceResponse"
]