...
class ExecutionResult:
    ...
    def __init__(self, success: bool, result: Any = None, stdout: str = "", 
                 stderr: str = "", error: str = "", execution_time: float = 0.0,
                 plots: List[str] = None, display_data: List[Dict] = None):
        ...
        try:
            ...
            if isinstance(obj, pd.DataFrame):
                return {
                    ...
                    "full_repr": str(obj) if len(obj) <= 100 else f"{str(obj[:5])}\\n...\\n{str(obj[-5:])}"
                }
        except ImportError:
            pass
        ...
        try:
            ...
            if isinstance(obj, np.ndarray):
                return {
                    ...
                    "shape": obj.shape,
                    "dtype": str(obj.dtype),
                    "data": obj.tolist() if obj.size <= 100 else "Array too large to display",
                    "full_repr": repr(obj)
                }
        except ImportError:
            pass
...
class PythonExecutorMCPProvider:
    ...
    DANGEROUS_IMPORTS = {
        ...
        'telnetlib', 'webbrowser', '__import__', 'eval', 'exec', 'compile'
    }
    ...
    def _initialize_session(self):
        ...
        if not IPYTHON_AVAILABLE:
            raise ImportError("IPython is required for Python execution. Install with: uv add ipython")
        ...
        try:
            self._shell.run_cell("from loads import LoadSet, LoadCase, PointLoad, ForceMoment")
    ...
    def _capture_execution(self, code: str) -> ExecutionResult:
        ...
        if self._shell is None:
            return ExecutionResult(
                success=False, 
                error="IPython shell not initialized"
            )
        ...
        try:
            ...
            for name, value in user_ns.items():
                if (not name.startswith('_') and 
                    not callable(value) or 
                    name in ['LoadSet', 'LoadCase', 'PointLoad', 'ForceMoment']):
                    try:
                        variables[name] = {
                            "type": type(value).__name__,
                            "value": repr(value)[:200] + ("..." if len(repr(value)) > 200 else ""),
            ...
            return {
                "success": False,
                "error": f"Failed to list variables: {str(e)}"
            }
    ...
    def configure_security(self, enable_security: bool = True, execution_timeout: int = 30) -> dict:
        """
