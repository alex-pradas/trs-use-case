...
async def demo_basic_processing():
    ...
    if result["success"]:
        ...
        for file_info in result["downloaded_files"]:
            print(
                f"  - {Path(file_info['local_path']).name} ({file_info['size']} bytes)"
            )
...
async def demo_comparison_workflow():
    ...
    agent = ScriptGenerationAgent(
        server, output_directory=Path("demo_outputs/comparison")
    )
...
async def demo_unit_analysis_workflow():
    ...
    server = create_mcp_server()
    agent = ScriptGenerationAgent(
        server, output_directory=Path("demo_outputs/unit_analysis")
    )
